...
export function Detail({item,onBack,onFavorite,onCategory,onDelete}:{item:Screenshot;onBack:()=>void;onFavorite:()=>void;onCategory:(value:string)=>void;onDelete:()=>Promise<boolean>}){
 ...
 const confirmDelete=()=>Alert.alert('Delete screenshot?','Android will ask you to confirm permanent deletion.',[
  ...
  {text:'Continue',style:'destructive',onPress:()=>{void onDelete().then(ok=>{if(ok)onBack()})}},
 ]);
 return <View style={{flex:1}}>
  <View style={{height:56,paddingHorizontal:PAGE_PADDING,flexDirection:'row',alignItems:'center',borderBottomWidth:1,borderColor:c.line,backgroundColor:c.surface}}>
   <Pressable accessibilityRole="button" accessibilityLabel="Go back" onPress={onBack} hitSlop={8} style={{width:40,height:40,alignItems:'center',justifyContent:'center',borderRadius:12,backgroundColor:c.pale,marginRight:12}}><Icon name="arrow-back" size={21} color={c.accent}/></Pressable>
   ...
   <Pressable accessibilityRole="button" accessibilityLabel={item.favorite?'Remove favorite':'Add favorite'} onPress={onFavorite} hitSlop={8} style={{width:40,height:40,alignItems:'center',justifyContent:'center'}}><Icon name={item.favorite?'star':'star-border'} size={23} color={c.accent}/></Pressable>
  </View>
